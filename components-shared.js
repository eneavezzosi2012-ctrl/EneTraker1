...
function GameStatsModal({onSave,onCancel}){
  ...
  const STAT_COLORS_LOCAL={
    "Punti":"#0A84FF","Assist":"#4db8ff","Rimbalzi":"#1EC96A",
    "Palle rubate":"#E09818","Falli":"#8055CC",
  };
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:400,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onCancel}>
      <div onClick={e=>e.stopPropagation()}
        ...
        <button onClick={()=>setSkipped(s=>!s)}
          style={{width:"100%",padding:"10px 12px",borderRadius:11,border:"1px solid "+(skipped?"rgba(255,204,0,0.4)":"rgba(255,255,255,0.08)"),background:skipped?"rgba(255,204,0,0.08)":"rgba(255,255,255,0.04)",color:skipped?"#E09818":"rgba(255,255,255,0.6)",fontSize:12,fontWeight:600,cursor:"pointer",marginBottom:12,textAlign:"left",display:"flex",alignItems:"center",gap:8}}>
        ...
        {!skipped&&(<>
          {/* Risultato finale */}
          <div style={{padding:"10px 11px",background:"rgba(10,132,255,0.04)",border:"1px solid rgba(10,132,255,0.18)",borderRadius:11,marginBottom:12}}>
            <div style={{fontSize:10,color:"#0A84FF",fontWeight:700,letterSpacing:.5,marginBottom:7}}>RISULTATO FINALE (opzionale)</div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{flex:1}}>
                ...
                <input type="number" inputMode="numeric" value={scoreMia} onChange={e=>setScoreMia(e.target.value)} placeholder="0"
                  style={{width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,color:"#0A84FF",padding:"7px 9px",fontSize:14,fontWeight:700,outline:"none",textAlign:"center",fontFamily:"var(--font-mono)"}}/>
              </div>
        ...
        <div style={{display:"flex",gap:8,marginTop:14}}>
          ...
          <button onClick={handleSave} style={{flex:2,padding:"11px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#0A7AE8,#0A84FF)",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",letterSpacing:.3}}>SALVA ✓</button>
        </div>
      </div>
    </div>
  );
