function NutritionTab({customMealPlan,meals,setMeals,water,setWater,profile,foodStars,setFoodStars,skippedMeals,setSkippedMeals}){
  ...
  const sc=s=>s>=4?"#1EC96A":s>=3?"#E09818":s?"#3360EE":"rgba(255,255,255,0.2)";
  const lbl=s=>(["","Pessimo","Male","Normale","Bene","Ottimo"])[Math.round(s)]||"";
  return(<>
    <div className="card cb">
      <div style={{fontSize:10,color:"#4da6ff",fontWeight:600,letterSpacing:.5,marginBottom:11}}>IDRATAZIONE</div>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:13}}>
        <div>
          <div className="H" style={{fontSize:36,color:"#4da6ff",lineHeight:1,fontWeight:700}}>{wMl}ml</div>
        ...
        <div style={{flex:1}}>
          <div className="pbar" style={{height:8}}><div className="pf" style={{width:Math.min((wMl/2000)*100,100)+"%",background:"linear-gradient(90deg,#0A84FF,#64D2FF)"}}/></div>
      ...
      <div style={{display:"flex",gap:7}}>
        ...
        <button className="wb" onClick={()=>{haptic.medium();setWater(0);}} style={{color:"#0A84FF",borderColor:"rgba(10,132,255,0.2)"}}>reset</button>
      </div>
    </div>
    {MEALS.map((meal,i)=>{
      ...
      return(
        <div key={meal} className="card" style={{opacity:isMealSkipped?0.55:1,borderColor:isMealSkipped?"rgba(255,204,0,0.3)":"rgba(255,255,255,0.065)"}}>
          <div style={{display:"flex",alignItems:"center",gap:11}}>
            ...
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                ...
                {isMealSkipped&&<span style={{fontSize:9,color:"#E09818",fontWeight:700,background:"rgba(255,204,0,0.12)",padding:"2px 6px",borderRadius:6,letterSpacing:.3}}>⚡ SALTATO</span>}
              </div>
              {!isMealSkipped&&<StarRating value={stars} onChange={v=>setMealStars(p=>({...p,[meal]:v}))}/>}
              {isMealSkipped&&<div style={{fontSize:11,color:"rgba(255,204,0,0.5)"}}>Non conta nella media</div>}
            </div>
            <div style={{textAlign:"right",flexShrink:0,display:"flex",flexDirection:"column",gap:5,alignItems:"flex-end"}}>
              <button onClick={toggleMealSkip} style={{background:isMealSkipped?"rgba(255,204,0,0.15)":"rgba(255,255,255,0.04)",border:"1px solid "+(isMealSkipped?"rgba(255,204,0,0.4)":"rgba(255,255,255,0.08)"),borderRadius:8,color:isMealSkipped?"#E09818":"rgba(255,255,255,0.3)",fontSize:12,padding:"4px 9px",cursor:"pointer",fontWeight:700,transition:"all .2s"}}>⚡</button>
          ...
          {isOpen&&plan&&(
            <div style={{marginTop:11,paddingTop:11,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
              <div style={{background:"rgba(10,132,255,0.06)",border:"1px solid rgba(10,132,255,0.1)",borderRadius:9,padding:"7px 11px",marginBottom:9}}>
                <div style={{fontSize:10,color:"#0A84FF",fontWeight:600,letterSpacing:.5,marginBottom:2}}>TARGET</div>
              ...
              {plan.warning&&<div style={{marginTop:8,background:"rgba(255,204,0,0.05)",border:"1px solid rgba(255,204,0,0.1)",borderRadius:9,padding:"7px 11px"}}><div style={{fontSize:11,color:"rgba(255,204,0,0.72)",lineHeight:1.4}}>{plan.warning}</div></div>}
            </div>
          )}
        </div>
      );
    })}
    <div className="card" style={{borderColor:"rgba(10,132,255,0.2)",background:"rgba(10,132,255,0.04)"}}>
      <div style={{fontSize:10,color:"#0A84FF",fontWeight:600,letterSpacing:.5,marginBottom:7}}>💪 POST-ALLENAMENTO (entro 30-45 min)</div>
      {["🥛 Latte 300ml + 3 cucchiai cacao + 2 fette pane con miele","🥛 Yogurt greco (vasetto grande) + banana + miele","🍳 2-3 uova + pane + succo di frutta"].map((t,i)=><div key={i} style={{fontSize:12,color:"rgba(255,255,255,0.45)",lineHeight:1.55,marginBottom:4,paddingLeft:8,borderLeft:"2px solid rgba(10,132,255,0.2)"}}>{t}</div>)}
    </div>
  </>);
}
