function TrainingTab({sched,exercises,toggleEx,skills,toggleSk,compEx,compSk,total,allDone,trainingDone,markGameDone,skippedEx,setSkippedEx,customSchedule}){
  ...
  return(<>
    {isGame?(
      <div className="card" style={{background:"linear-gradient(135deg,rgba(10,132,255,0.1),rgba(8,9,16,0.96))",border:"1px solid rgba(10,132,255,0.22)",textAlign:"center",padding:"30px 20px"}}>
        <div style={{fontSize:54}}>🏆</div>
        <div className="H" style={{fontSize:38,color:"#0A84FF",marginTop:10}}>GAME DAY!</div>
        ...
        {!trainingDone?<button onClick={markGameDone} className="pulse" style={{marginTop:16,background:"#2563EB",border:"none",borderRadius:13,color:"#fff",fontFamily:"'DM Sans'",fontSize:14,fontWeight:700,padding:"12px 30px",cursor:"pointer"}}>HO GIOCATO ✓</button>
        :<div style={{marginTop:13,background:"rgba(0,201,126,0.09)",border:"1px solid rgba(0,201,126,0.2)",borderRadius:12,padding:10}}><span className="H" style={{fontSize:14,color:"#1EC96A"}}>PARTITA GIOCATA!</span></div>}
      </div>
    ):isRest?(
      ...
      <div className="card co" style={{background:"linear-gradient(135deg,rgba(10,132,255,0.08),rgba(8,9,16,0.96))"}}>
        <div style={{fontSize:10,color:"#0A84FF",fontWeight:600,letterSpacing:.8,marginBottom:5}}>PIANO DI OGGI</div>
        ...
        {total>0&&(
          <div style={{marginTop:11}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
              ...
              <span style={{fontSize:11,color:allDone?"#1EC96A":"#0A84FF",fontWeight:600}}>{compEx+compSk}/{total} · {pct}%</span>
            </div>
            <div className="pbar" style={{height:5}}><div className="pf" style={{width:pct+"%",background:allDone?"linear-gradient(90deg,#1EC96A,#1EC96A)":"linear-gradient(90deg,#0A84FF,#0A84FF)"}}/></div>
          </div>
        )}
        {allDone&&<div style={{marginTop:10,background:"rgba(0,201,126,0.08)",border:"1px solid rgba(0,201,126,0.2)",borderRadius:11,padding:9,textAlign:"center"}}><span className="H" style={{fontSize:14,color:"#1EC96A"}}>COMPLETATO!</span></div>}
      </div>
    ))
    }
    {(()=>{
      ...
      return blocks.map((block,bi)=>{
        ...
        <div key={block.id||bi} className="card">
          ...
          <div className="pbar" style={{marginBottom:12}}><div className="pf" style={{width:(items.length>0?(compB/items.length)*100:0)+"%",background:"linear-gradient(90deg,#0A7AE8,#0A84FF)"}}/></div>
          {items.map((it,ii)=>{
            ...
            <div key={itemKey} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10,opacity:isSkipped?0.45:1}}>
              ...
              {isSkipped&&<div style={{width:24,height:24,borderRadius:7,background:"rgba(255,204,0,0.1)",border:"1px solid rgba(255,204,0,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11}}>⚡</div>}
              <span style={{fontSize:16}}>{it.icon||block.icon||"💪"}</span>
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:600,color:isSkipped?"rgba(255,204,0,0.5)":isDone?"#0A84FF":"rgba(255,255,255,0.8)",textDecoration:isSkipped||isDone?"line-through":"none"}}>{it.name||"(senza nome)"}</div>
                <div style={{fontSize:11,color:isSkipped?"rgba(255,204,0,0.35)":"rgba(255,255,255,0.22)"}}>{isSkipped?"non conta":it.sets||""}</div>
              </div>
              <button onClick={toggleSkipItem} style={{background:isSkipped?"rgba(255,204,0,0.12)":"rgba(255,255,255,0.04)",border:"1px solid "+(isSkipped?"rgba(255,204,0,0.3)":"rgba(255,255,255,0.07)"),borderRadius:7,color:isSkipped?"#E09818":"rgba(255,255,255,0.25)",fontSize:11,padding:"3px 7px",cursor:"pointer",fontWeight:700}}>⚡</button>
            </div>
            );
          })}
        </div>
    ...
    <div className="card cb">
      <div style={{fontSize:10,color:"#4da6ff",fontWeight:600,letterSpacing:.5,marginBottom:10}}>PIANO SETTIMANA</div>
      {(()=>{
